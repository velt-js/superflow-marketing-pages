...
/**
 * Nav logo the inlined copy in `logo.mjs` is derived from.
 * `npm run og:check-logo` compares the two and fails on drift.
 */
...
/**
 * Committed Urbanist TTFs, relative to this module.
 *
 * `next/og` rasterizes with Satori, which reads ttf/otf/woff but NOT woff2 -
 * so these are the static `format('truetype')` files from Google Fonts, not
 * the woff2 a browser would pick up. They are committed rather than fetched
 * because the `/api/og` route needs them at request time; downloading per
 * cold start would put a third party in the render path.
 */
export const FONT_FILES = {
  [FONT_WEIGHT_REGULAR]: "fonts/urbanist-400.ttf",
  [FONT_WEIGHT_BOLD]: "fonts/urbanist-700.ttf",
};
