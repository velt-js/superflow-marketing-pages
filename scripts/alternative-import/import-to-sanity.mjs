 ...
 *
 * Reads scripts/alternative-import/framer-alt-sanity.json. Recursively walks
 ...
 * createOrReplace persists the doc keyed `alt-<slug>`.
 *
 * Filename extension is preserved on upload, so SVGs land as SVGs.
 *
 * Usage:
 *   SANITY_API_TOKEN=<token> node scripts/alternative-import/import-to-sanity.mjs
 *   DRY_RUN=1 node scripts/alternative-import/import-to-sanity.mjs
 *   LIMIT=1   node scripts/alternative-import/import-to-sanity.mjs
 */
...
const docs = JSON.parse(readFileSync(here("framer-alt-sanity.json"), "utf8"));
...
async function main() {
  ...
  for (const doc of docs) {
    ...
    if (DRY_RUN) {
      console.log(`  [dry] alternativePage ${doc._id}`);
    } else {
      await client.createOrReplace(toPersist);
      console.log(`  ✓ alternativePage ${doc._id}`);
    }
  }
