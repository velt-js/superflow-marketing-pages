#!/usr/bin/env node
/**
 * Seed reviewPage-pdf-review in Sanity, including the two per-feature
...
async function main() {
  ...
  const card1Image = await uploadAsset(`${fcReview}/c1-pdf.png`);
  ...
  const collab = [
    {
      ...
      preview: `${cFig}/preview-1-pdf.png`,
    },
    {
  ...
  const doc = {
    _id: "reviewPage-pdf-review",
    _type: "reviewPage",
    title: "PDF Review",
    slug: { _type: "slug", current: "pdf-review" },
    feature: "pdf",
    hero: {
      headlineLine1: "Ship PDF Documents",
      subheading: "Get approved with fewer rounds of reviews. Get back to creating.",
      personaLeft: { label: "Writer", color: "#3DB7E4" },
    ...
    faqFormatsAnswer: "PDF files including form-fillable PDFs.",
    metaTitle: "PDF Review — Superflow by Velt",
    metaDescription:
      "Ship PDF documents impossibly fast. Comment, prioritise, approve and sync PDF reviews with your existing tools.",
  };
