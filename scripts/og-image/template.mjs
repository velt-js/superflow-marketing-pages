...
/**
 * Load the site nav logo and prepare it for rasterization.
 *
 * The committed SVG paints its fills through `var(--fill-0, #HEX)`, which the
 * rasterizer cannot resolve, and declares percentage dimensions. Both are
 * rewritten here so the one committed logo stays the single source of truth
 * instead of forking a second copy for social cards.
 *
 * @param {string} projectRoot - Repository root.
 * @returns {Promise<{dataUri: string, aspectRatio: number}>} Inline-able SVG
 *   plus its intrinsic width/height ratio.
 */
export async function loadLogo(projectRoot) {
  try {
    const raw = await readFile(join(projectRoot, LOGO_SVG_PATH), "utf8");

    // `fill="var(--fill-0, #FFCD2E)"` -> `fill="#FFCD2E"`.
    ...
    // Percentage dimensions and `preserveAspectRatio="none"` make the
    // rasterized output unpredictable; pin both to the viewBox instead.
    svg = svg
    ...
    throw new Error(`Could not load logo from ${LOGO_SVG_PATH}: ${error?.message ?? error}`);
  }
}
