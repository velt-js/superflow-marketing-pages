 ...
 *
 * Reads scripts/comparisons-import/framer-cmp-sanity.json. Recursively walks
 ...
 * createOrReplace persists the doc keyed `cmp-<slug>`.
 *
...
const docs = JSON.parse(readFileSync(here("framer-cmp-sanity.json"), "utf8"));
...
async function main() {
  ...
  for (const doc of docs) {
    ...
    if (DRY_RUN) {
      console.log(`  [dry] comparisonPage ${doc._id}`);
    } else {
      await client.createOrReplace(toPersist);
      console.log(`  ✓ comparisonPage ${doc._id}`);
    }
  }
